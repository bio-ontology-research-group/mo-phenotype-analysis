# Sarah M. Alghamdi
#------------------------------------
# this is a modified version of the OWL2Vec* implimentaion from : https://github.com/KRR-Oxford/OWL2Vec-Star
#------------------------------------
...
if 'URI_Doc' in config['DOCUMENT'] and config['DOCUMENT']['URI_Doc'] == 'yes':
    ...
    print("saving walks")
    with open(os.path.join(config['DOCUMENT']['cache_dir'], 'document_sentences_walks.txt'), 'w') as f:
    ...
    print("saving axioms sentences")
    with open(os.path.join(config['DOCUMENT']['cache_dir'], 'document_sentences_axioms.txt'), 'w') as f:
...
with open(sys.argv[2]) as f:
    ...
    if 'annotation_extra' in config['DOCUMENT']:
        ...
        for sentence in URI_Doc:
            ...
            if(len(parts)>0 and "http" in parts[0]):
                if parts[0] in classes_to_add:
                    for annot in classes_to_add[parts[0]]:
                        additional_lines.append(annot+' '+l.rstrip()+'\n')

        with open(os.path.join(config['DOCUMENT']['cache_dir'], 'document_sentences_additional_lines.txt'), 'w') as f:
            for sentence in additional_lines:
                for w in sentence:
                    f.write('%s ' % w)
                f.write('\n')
            f.close()
        ...
        URI_Doc = walk_sentences + axiom_sentences + additional_lines
