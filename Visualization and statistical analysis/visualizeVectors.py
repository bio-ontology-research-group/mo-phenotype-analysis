...
file_name = 'Data/vectors.text'
phenotpes = 'Data/Fly_test_input_only_FBbtAB.txt'
phenotpes_annotaions = ['FBcv','FBbtAb']
...
with open("../fly_human_mapping.json",'r') as f:
	F_human = json.load(f)
...
for p in content:
	...
	if ls[0] in p_genes:
		p_pheno.add(ls[1])
		if("FBbt" in ls[1]):
			p_genes_fbbt.add(ls[0])
		if("FBcv" in ls[1]):
			p_genes_fbcv.add(ls[0])
	else:
		if 'FBgn' in ls[0]:
			n_pheno.add(ls[1])
			if("FBbt" in ls[1]):
				n_genes_fbbt.add(ls[0])
			if("FBcv" in ls[1]):
				n_genes_fbcv.add(ls[0])

n_genes = set([])
for key in vectors:
	if 'FBgn' in key and key not in p_genes:
		n_genes.add(key)
...
with open('pos', 'w') as fb:
        json.dump(pos,fb)
with open('neg', 'w') as fb:
        json.dump(neg,fb)

with open('frequencies_neg', 'w') as fb:
        json.dump(frequencies_neg,fb)
with open('frequencies_pos', 'w') as fb:
...
print("FBcv", len([x for x in p_pheno if 'FBcv' in x]), 'genes', len(p_genes_fbcv))
print("FBbtAb" ,  len([x for x in p_pheno if 'FBbtAB' in x]) , 'genes', len(p_genes_fbbt))
print("non-disease-associated genes phenotypes", len(n_pheno))
print("FBcv", len([x for x in n_pheno if 'FBcv' in x]),'genes', len(n_genes_fbcv) )
print("FBbtAb" ,  len([x for x in n_pheno if 'FBbtAB' in x]) , 'genes', len(n_genes_fbbt))
...
#P_P = { key:value for key,value in vectors.items() if key in p_pheno}
#N_P = { key:value for key,value in vectors.items() if key in n_pheno}
#PN_P_I = { key:value for key,value in vectors.items() if (key in p_pheno and key in n_pheno) }
