...
for mapping in content:
	if (len(mapping.split())>1):
		ls = mapping.split()
		disease2annotations[ls[0]] = set(ls[1:])
...
included_annotaions={}
for disease in disease2annotations:
	ls = [disease]
	ls2 = [disease]
	for annot in disease2annotations[disease]:
	...
	if(len(ls)>1):
		print("\t".join(ls2))
		included_annotaions[disease]=set(ls[1:])
...
for mapping in content:
	ls = mapping.split()
	associated_diseases.add(ls[0])
Diseases_sum_IC_associated = []
Diseases_sum_IC_non_associated = []


for disease in disease2annotations:
	annotations_IC = []
	for annotation in disease2annotations[disease]:
		if annotation in annotaion2IC:
			annotations_IC.append(annotaion2IC[annotation])
		else:
			print(annotation)

	if disease in associated_diseases:
		Diseases_sum_IC_associated.append(sum(annotations_IC))
	else:
		Diseases_sum_IC_non_associated.append(sum(annotations_IC))



plt.hist(Diseases_sum_IC_associated, bins=n_bins , density=1, alpha=0.5, label='associated diseases')
plt.hist(Diseases_sum_IC_non_associated, bins=n_bins, density=1 ,alpha=0.5, label='non-associated diseases')
...
Diseases_sum_IC_associated.sort()
Diseases_sum_IC_non_associated.sort()

t2, p2 = stats.ttest_ind(a=Diseases_sum_IC_associated,b=Diseases_sum_IC_non_associated, alternative="greater")
print("t = " + str(t2))
print("p = " + str(p2))
